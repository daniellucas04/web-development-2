        ...
        <div class="card" style="width:40rem;">
            <div class="card-body">
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    ...
                    $sql = "SELECT usuario, senha FROM enfermeiro WHERE usuario = :usuario";
                    $select = $conn->prepare($sql);
                    $select->execute(['usuario' => $data['usuario']]);
                    $dados = $select->fetch(PDO::FETCH_ASSOC);
        
                    if ($dados == false) {
                        echo "<div class='alert alert-danger'>Usuário não encontrado, <a href='cadastrar_enfermeiro.php'>cadastre-se aqui</a>!</div>";
                        $error = true;
                    }
